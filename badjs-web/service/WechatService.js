...
var mdb = orm.connect(mysqlUrl, function (err, db) {
    ...
    const sql = 'select s.*, a.userName, a.name from b_quality as s, b_apply as a where s.badjsid=a.id and a.status=1 and a.online=2 and s.pv>a.limitpv and s.rate > 0.5 and s.date=' + yesterday + ' order by s.rate + 0 desc;';
